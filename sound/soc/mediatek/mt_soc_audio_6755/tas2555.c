...
#define AUDIOSERVERKILL
extern uint32 mBlockSampleRate[AUDIO_ANALOG_DEVICE_INOUT_MAX];
#ifdef AUDIOSERVERKILL
uint32 mStartTas2555Count;//shuangadd
#endif
...
static unsigned int p_tas2555_startup_data[] = {
	...
	TAS2555_UDELAY, 2000,		//delay 2ms
	TAS2555_CLK_ERR_CTRL1, 0x0B,	//disable clock error detection on PLL <--- CHANGED (MCLK & WDCLK)//oliver add
	0xFFFFFFFF, 0xFFFFFFFF
};
...
static unsigned int p_tas2555_shutdown_data[] = {
	TAS2555_CLK_ERR_CTRL1, 0x00,	//disable clock error detection on PLL //oliver add
...
#ifdef AUDIOSERVERKILL
	pr_warn("tas2555_startup mStartTas2555Count = %d\n",mStartTas2555Count);
	mt63xx_codec_startup(substream, dai);
	mStartTas2555Count++;
#endif
	return 0;
}
...
struct snd_soc_codec *codec = dai->codec;
  ...
  struct tas2555_priv *pTAS2555 = snd_soc_codec_get_drvdata(codec);
	...
	pr_warn("tas2555_shutdown mStartTas2555Count = %d\n",mStartTas2555Count);
	mStartTas2555Count--;
	if (mStartTas2555Count > 0){
		//do nothing
	} 
	else {
	pr_warn("tas2555_shutdown off\n");
	...
	if (bEnable) {
		if (!pTAS2555->mbPowerUp) {
			...
			if ((nValue2 & 0xff) != TAS2555_SAFE_GUARD_PATTERN) {
					...
					nResult = -1;
					pTAS2555->mnErrorCode |= TAS2555_ERROR_RESET;
					goto end;
			}
...
int tas2555_get_errcode(struct tas2555_priv *pTAS2555, unsigned int *pErrCode)
{
	...
	if (nResult >= 0) {
		...
		if (nValue1 & 0x20) {
			errcode |= TAS2555_ERROR_CLKPRESENT_WCLK;
...
#if 0//def MTK_PLATFORM_DRIVER //audioserverkill
...
{	
	...
	if ((nDevInt1Status & 0xfc) != 0) {		
		/* in case of INT_OC, INT_UV, INT_OT, INT_BO, INT_CL, INT_CLK1, INT_CLK2 */		
		dev_err(pTAS2555->dev, "critical error INT Status: 0x%x\n", nDevInt1Status);
		if ((nDevInt1Status & 0x20) && (nDevInt1Status & 0x04)) {	
			dev_err(pTAS2555->dev, "tas2555 kill audio server: 0x26\n");
			pTAS2555->mnErrorCode |= TAS2555_ERROR_CLKPRESENT_WCLK;	
...
//  regmap_config = &tas2555_i2c_regmap;
    ...
    #if 1
    pTAS2555->mnGpioINT=52;//add this line
    #endif
	if (gpio_is_valid(pTAS2555->mnGpioINT)) {		
