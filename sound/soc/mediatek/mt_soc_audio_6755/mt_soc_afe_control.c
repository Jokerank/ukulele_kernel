...
{
     printk("%s APLL1Counter = %d APLL2Counter = %d  SampleRate = %d\n", __func__, APLL1Counter,
...
void DisableALLbySampleRate(uint32 SampleRate)
{
	...
	if (GetApllbySampleRate(SampleRate) == Soc_Aud_APLL1) {
		...
		if(APLL1Counter <= 0)//fix speaker no sound by oliverchen 20161028
		return;
		...
		if (APLL1Counter == 0) {
	     ...
	     if(APLL2Counter <= 0)//fix speaker no sound by oliverchen 20161028
	      return;
		/* disable APLL2 */
		APLL2Counter--;
		printk("%s APLL2Counter = %d \n", __func__,APLL2Counter);
