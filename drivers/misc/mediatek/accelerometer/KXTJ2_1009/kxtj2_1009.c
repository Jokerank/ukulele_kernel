...
static int KXTJ2_1009_ReadData(struct i2c_client *client, s16 data[KXTJ2_1009_AXES_NUM])
{
	...
	int err = 0;
	int i;
/*	//<2017/06/02-ShermanWei,Fix coverity check issue
	if(NULL == client)
	{
		err = -EINVAL;
	}
...
static ssize_t wufe_fun(struct device_driver *ddri, char *buf)
{
	...
	write_retry(obj->client, 0x1B, 0x40);
	...
	write_retry(obj->client, 0x1B, 0xC0);
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				if(((value == 0) && (sensor_power == false)) ||((value == 1) && (sensor_power == true)))
				{
					...
					write_retry(obj->client, 0x1B, 0xC0);
					//read_retry(obj->client, 0x1A, &data8);
					GSE_LOG("[Rick] Gsensor KXTJ2_1009, disable interrupt \n");
				}
				else
				{
					enable_status = !sensor_power;
					if (atomic_read(&priv->suspend) == 0)
					{
						...
						else
						{
							...
							//enable interrupt
							GSE_LOG("[Rick] Gsensor KXTJ2_1009, enable interrupt \n");
							write_retry(obj->client, 0x1B, 0x00);
							write_retry(obj->client, 0x1B, 0x42);
							write_retry(obj->client, 0x1D, 0x06);
							write_retry(obj->client, 0x1F, 0x3F);
							write_retry(obj->client, 0x29, 0x05);//WAKEUP_TIMER
							write_retry(obj->client, 0x6A, 0x10);//RexLu 20170218 Modify WAKEUP_THRESHOLD for CTS7.0_R6 and Doze.
							write_retry(obj->client, 0x1E, 0x30);
...
static irqreturn_t kxtj2_1009_irq_handler(int irq, void *handle)
{
	...
	printk("[Rick] Doze kxtj2_1009_irq_handler, G sensor SHAKE \n");
	return IRQ_HANDLED;
}
