	...
	case PLUG_OUT:
		...
		if (current_status == 0) {
			...
			if (current_status == 0 && show_icon_delay != 0) {
				...
				/*accdet_auxadc_switch(0);*/
				//<2017/2/17-jackhue, accdet_status wrong for non-CTIA headset DMS09526421
				if (220 > pin_adc_value && pin_adc_value > 90) {	/*90mv   ilegal headset*/
				//<2017/2/17-jackhue
...
#ifdef CONFIG_ACCDET_PIN_RECOGNIZATION
	pmic_pwrap_write(ACCDET_DEBOUNCE0, cust_headset_settings->debounce0);
    //<2017/3/2-jackhue, Decrease debounce time for headset detection
