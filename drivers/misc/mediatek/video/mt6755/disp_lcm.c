...
#include "disp_lcm.h"
//<2017/04/11-stevenchen, Fix mess screen during fota process.
#include <linux/fs.h>
//>2017/04/11-stevenchen
...
//<2017/04/11-stevenchen, Fix mess screen during fota process.
extern int emergency_remount_n;
static int fota_boot=0,	check_count=0;
int check_fota_boot(void)
{
    struct file *fd = NULL;
	
    fd = filp_open("/cache/recovery/command", O_RDONLY, 0);
    if (IS_ERR(fd)) {
        return 0;
    }
    else{
...
int disp_lcm_suspend(disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->suspend) {
		    ...
		    {
			check_count=1;
		        printk("check fota boot \n");
			if(emergency_remount_n!=1)
			{				        
			        if(check_fota_boot())
			        {
			            printk("Skip suspend \n");
