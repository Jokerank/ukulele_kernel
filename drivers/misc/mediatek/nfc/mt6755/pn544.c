...
typedef struct st_pn544_dev
{
	...
	//>2017/02/09-JackHu-remove mutex and modify i2c funrction to use i2c_transfer 
    //struct mutex        read_mutex;
	//>2017/01/14-JackHu-  avoid KE crash for error access non-DMA mode memory in multi-thread case (ALPS03114474)
    ...
    size_t count, loff_t *offset)
{
	...
	msg.addr = pn544_dev->client->addr;
	msg.flags = (pn544_dev->client->flags & I2C_M_TEN) | I2C_M_RD;
	msg.len = count;

#if defined( I2C_DMA_USAGE )
	msg.buf = (__u8 *)I2CDMAReadBuf_pa;
#ifdef CONFIG_MTK_I2C_EXTENSION
	msg.timing = NFC_CLIENT_TIMING;
...
//    }
	msg.buf = (__u8 *)tmp;
#endif
	ret = i2c_transfer(pn544_dev->client->adapter, &msg, 1);
	if (ret == 1)
		ret = count;
    ...
    size_t count, loff_t *offset)
{
	...
	memset(&msg, 0, sizeof(msg));
	msg.addr = pn544_dev->client->addr;
	msg.flags = (pn544_dev->client->flags & I2C_M_TEN);
	msg.len = count;
  ...
  //pn544_dev->client->addr     |= I2C_ENEXT_FLAG;
    ...
    /* Write data */
#if defined( I2C_DMA_USAGE )
	msg.buf = (__u8 *)I2CDMAWriteBuf_pa;
#ifdef CONFIG_MTK_I2C_EXTENSION
	msg.ext_flag = I2C_DMA_FLAG;
#endif
