...
////...    INIT_DELAYED_WORK(&obj->eint_work, epl_sensor_eint_work);
    ...
    if((err = misc_register(&epl_sensor_device)))
    {
        ...
        printk("sherman +++++ epl_sensor_i2c_probe.fail_1 +++++ \n");
    ...
    if((err = epl_sensor_create_attr(&epl_sensor_init_info.platform_diver_addr->driver)))
    {
        ...
        printk("sherman +++++ epl_sensor_i2c_probe.fail_2 +++++ \n");
	...
	if(err)
	{
		...
		printk("sherman +++++ epl_sensor_i2c_probe.fail_3 +++++ \n");
		goto exit_create_attr_failed;
	}
	...
	if(err)
	{
		...
		printk("sherman +++++ epl_sensor_i2c_probe.fail_4 +++++ \n");
		goto exit_create_attr_failed;
	}
...
//>2015/12/02 ShermanWei,
	...
	{
		APS_ERR("register fail = %d\n", err);
		printk("sherman +++++ epl_sensor_i2c_probe.fail_5 +++++ \n");
		goto exit_create_attr_failed;
	}
	...
	{
		APS_ERR("tregister fail = %d\n", err);
		printk("sherman +++++ epl_sensor_i2c_probe.fail_6 +++++ \n");
		goto exit_create_attr_failed;
	}
...
printk("sherman +++++ epl_sensor_i2c_probe.5 +++++ \n");
#if defined(CONFIG_HAS_EARLYSUSPEND)
