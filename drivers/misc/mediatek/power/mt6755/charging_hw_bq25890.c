...
extern unsigned int enable_charger_hiz_mode;
// >>> 2017/05/24-youchihwang. Battery. Release charger HIZ mode switch to user space.
// <<< 2017/06/30-youchihwang. Enabling absolute VINDPM under META mode.
...
// <<< 2016/11/29-youchihwang. Battery. Clear useless code.
#if 0
...
// >>> 2016/11/29-youchihwang. Battery. Clear useless code.
	...
	/*Vbus current limit */
// <<< 2016/11/29-youchihwang. Battery. Setting input current limit.
#if 0
	bq25890_config_interface(bq25890_CON0, 0x3F, 0x3F, 0);	/* input current limit, IINLIM, 3.25A */
#else
	bq25890_config_interface(bq25890_CON0, 0x1D, 0x3F, 0);	/* input current limit, IINLIM, 1.55A */
#endif
// >>> 2016/11/29-youchihwang. Battery. Setting input current limit.
	...
	/*CV mode */
// <<< 2016/11/29-youchihwang. Battery. Setting CV.
#if 0
...
// <<< 2016/11/29-youchihwang. Battery. Setting boost current limit.
#if 0
	bq25890_config_interface(bq25890_CONA, 0x3, 0x7, 0);	/* boost current limit 1.3A */
#else
	bq25890_config_interface(bq25890_CONA, 0x0, 0x7, 0);	/* boost current limit 0.5A */
#endif
// >>> 2016/11/29-youchihwang. Battery. Setting boost current limit.
...
#else
// <<< 2016/11/29-youchihwang. Battery. disable ir_comp_resistance. disable ir_comp_vdamp.
#if 0
...
#else
	bq25890_config_interface(bq25890_CON5, 0x3, 0xF, 4);	/* precharge current default 256mA */
#endif
// >>> 2016/11/29-youchihwang. Battery. disable ir_comp_resistance. disable ir_comp_vdamp.
	...
	bq25890_config_interface(bq25890_CON7, 0x1, 0x1, 7);	/* enable ICHG termination detect */
        // <<< 2017/05/16-youchihwang. Battery. Correcting the mask of termination current setting.
        #if 0
...
#if 0
#else
static int charging_set_hiz_mode(void *data)
{
	int status = STATUS_OK;
	unsigned int en;
...
static int charging_read_hiz_mode(void *data)
{
	int status = STATUS_OK;
	unsigned int en;

	en = bq25890_read_en_hiz();
	*(unsigned int *) data = en;
...
static int charging_get_vbus(void *data)
{
	...
	int status = STATUS_OK;
        ...
        *(unsigned int *) data = (bq25890_get_vbus() * 100) + 2600;
...
// <<< 2017/04/26-youchihwang, Battery. The phone can't show Vcharger warning message.
static int charging_adc_start(void *data)
{
	int status = STATUS_OK;
        unsigned int enable = 0;

        enable = *(unsigned int *)data;
        bq25890_ADC_start(enable);
...
#if 0
#else
        charging_set_hiz_mode,
        charging_read_hiz_mode
#endif
        // >>> 2017/05/24-youchihwang. Battery. Release charger HIZ mode switch to user space.
};
