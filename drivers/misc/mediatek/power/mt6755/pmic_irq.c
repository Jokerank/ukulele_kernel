...
// <<< 2017/03/03-youchihwang. Battery. Getting the battery capacity of the charging animation under power off.
#if 0
        // original code
#else
        // modified code
extern int charging_animation_battery_capacity;
#endif
// >>> 2017/03/03-youchihwang. Battery. Getting the battery capacity of the charging animation under power off.
		...
		PMICLOG
		    ("timer_pos = %ld, timer_pre = %ld, timer_pos-timer_pre = %ld, long_pwrkey_press = %d\r\n",
		     timer_pos, timer_pre, timer_pos - timer_pre, long_pwrkey_press);
                // <<< 2017/02/21-youchihwang. Battery. Phone can't power on on 0%.
        #if 0
        if (long_pwrkey_press) {	/*500ms */
        #else
        // <<< 2017/03/16-youchihwang. Battery. Phone's request : can't power on on 0%.
		if (long_pwrkey_press && (charging_animation_battery_capacity > 0)) {	/*500ms */
        // >>> 2017/03/16-youchihwang. Battery. Phone's request : can't power on on 0%.
        #endif
        // >>> 2017/02/21-youchihwang. Battery. Phone can't power on on 0%.
			PMICLOG
 ...
 * Chrdet Int Handler
 ******************************************************************************/
// <<< 2017/01/26-dangyiwang. Turn off LED through kernel control. when pull out usb cable in phone power on.
...
// <<< 2017/01/26-dangyiwang. Turn off LED through kernel control. when pull out usb cable in phone power on.
	if (!upmu_get_rgs_chrdet()){
		printk("[dang test] %s(%d): need to close LED", __func__, __LINE__);
		mt65xx_leds_brightness_set(0,0);
		mt65xx_leds_brightness_set(1,0);
		mt65xx_leds_brightness_set(2,0);
	}
// >>> 2017/01/26-dangyiwang. Turn off LED through kernel control. when pull out usb cable in phone power on.
