...
// <<< 2017/03/31-youchihwang, Battery. DMS09529480. The battery power will decrease rapidly when the battery level is less than 20%.
// SM20N drop from 25% to 17% during 8h standby.
#if 0
#else
...
int BattThermistorConverTemp(int Res)
{
	...
	if (Res >= Batt_Temperature_Table[0].TemperatureR) {
        // <<< 2016/11/03-youchihwang. Battery. Releated Battery table. transfer from SM20_M
...
signed int get_dynamic_period(int first_use, int first_wakeup_time, int battery_capacity_level)
{
	...
	if (wake_up_smooth_time == 0)
        ...
        if (wake_up_smooth_time == 0)
                g_spm_timer = NORMAL_WAKEUP_PERIOD;
        else if(wake_up_smooth_time <= max_smooth_time)
                g_spm_timer = wake_up_smooth_time;
        else if(wake_up_smooth_time > max_smooth_time)
                g_spm_timer = max_smooth_time;
        #endif
        // >>> 2017/03/31-youchihwang, Battery. DMS09529480. The battery power will decrease rapidly when the battery level is less than 20%.

// <<< 2017/03/31-youchihwang, Battery. DMS09529480. The battery power will decrease rapidly when the battery level is less than 20%.
// SM20N drop from 25% to 17% during 8h standby.
// <<< 2016/12/09-youchihwang, Battery, Prevent battery capacity is not updated after long time so set a limit time.
#if 0
        //original code
#else
        if (g_spm_timer > NORMAL_WAKEUP_PERIOD)
                g_spm_timer = NORMAL_WAKEUP_PERIOD;
#endif
// >>> 2016/12/09-youchihwang, Battery, Prevent battery capacity is not updated after long time so set a limit time.
// >>> 2017/03/31-youchihwang, Battery. DMS09529480. The battery power will decrease rapidly when the battery level is less than 20%.
	...
	case FG_DAEMON_CMD_SET_POWEROFF:
		{
            ...
            if (BMT_status.bat_vol <= 3400) {
                ...
                bm_debug("[fg_res] FG_DAEMON_CMD_SET_POWEROFF, bat_vol > 3.4V, not allowed to shutdown\n");
            }
            // >>> 2017/01/17-youchihwang. Battery. Extending battery life. (The phase of 1% -> shutdown)
		}
		break;
