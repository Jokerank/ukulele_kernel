...
#endif
// <<< 2017/04/26-youchihwang, Battery. The phone can't show Vcharger warning message.
static unsigned int is_charger_ovp(unsigned int vbus);
// >>> 2017/04/26-youchihwang, Battery. The phone can't show Vcharger warning message.
// <<< 2017/05/24-youchihwang. Battery. Release charger HIZ mode switch to user space.
unsigned int enable_charger_hiz_mode = 0;
// >>> 2017/05/24-youchihwang. Battery. Release charger HIZ mode switch to user space.
/* ////////////////////////////////////////////////////////////////////////////// */
...
// <<< 2016/11/14-youchihwang. Battery. FP002219. IEEE 1725 (Battery Safety). Safety timer
#if 1
...
#endif
// <<< 2016/11/21-youchihwang. Battery. FP002219. IEEE 1725 (Battery Safety). Maintenance.
extern BATTERY_VOLTAGE_ENUM g_cv_voltage;
// >>> 2016/11/21-youchihwang. Battery. FP002219. IEEE 1725 (Battery Safety). Maintenance.
/* ////////////////////////////////////////////////////////////////////////////// */
...
// <<< 2016/12/19-youchihwang, Battery. Porting Qnovo feature
...
// <<< 2016/11/09-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
static int FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable = 0;
// >>> 2016/11/09-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
// <<< 2017/03/16-youchihwang. Battery. Phone's request : can't power on on 0%.
#if 0
       // original code
#else
       // modified code
       int charging_animation_battery_capacity = 0;
#endif
// >>> 2017/03/16-youchihwang. Battery. Phone's request : can't power on on 0%.
...
static int battery_property_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
        switch (psp) {
        case POWER_SUPPLY_PROP_MAX_CHARGE_CURRENT:
                return 1;
...
// <<< 2017/03/16-youchihwang. Battery. Phone's request : can't power on on 0%.
        ...
        // modified code
///////////////////////////////////////////////////////////////////////////////////////////////
//    Battery. Getting the battery capacity of the charging animation under power off.
///////////////////////////////////////////////////////////////////////////////////////////////
static ssize_t store_Charging_Animation_Battery_Capacity (struct device *dev,
                                                          struct device_attribute *attr,
                                                          const char *buf,
                                                          size_t size)
{
        if (kstrtouint(buf, 10, &charging_animation_battery_capacity) == 0) {
                battery_log (BAT_LOG_CRTI,
                             "Charging_Animation_Battery_Capacity : %d \n",
                             charging_animation_battery_capacity
                            );
        }
...
static DEVICE_ATTR(Charging_Animation_Battery_Capacity, 0744, NULL, store_Charging_Animation_Battery_Capacity);
#endif
// >>> 2017/03/16-youchihwang. Battery. Phone's request : can't power on on 0%.
...
#if 0
#else
static ssize_t show_Enable_Charger_HIZ_Mode (struct device *dev,
                                             struct device_attribute *attr,
                                             char *buf)
{
        int enable_charger_hiz_mode = 0;

        battery_charging_control(CHARGING_CMD_READ_HIZ_MODE, &enable_charger_hiz_mode);

        battery_log (BAT_LOG_CRTI, "Enable_Charger_HIZ_Mode : %d \n", enable_charger_hiz_mode);

        return sprintf(buf, "Enable_Charger_HIZ_Mode : %d\n", enable_charger_hiz_mode);
}

static ssize_t store_Enable_Charger_HIZ_Mode (struct device *dev,
                                              struct device_attribute *attr,
                                              const char *buf,
                                              size_t size)
{
        if (kstrtouint(buf, 10, &enable_charger_hiz_mode) == 0) {
                if (enable_charger_hiz_mode == 0) {
                        battery_charging_control(CHARGING_CMD_SET_HIZ_SWCHR, &enable_charger_hiz_mode);
                } else if (enable_charger_hiz_mode == 1) {
                        battery_charging_control(CHARGING_CMD_SET_HIZ_SWCHR, &enable_charger_hiz_mode);
                        battery_log (BAT_LOG_CRTI, "store_Enable_Charger_HIZ_Mode : %d, using vindpm \n", enable_charger_hiz_mode);
                } else if (enable_charger_hiz_mode == 2) {
                        //battery_charging_control(CHARGING_CMD_SET_HIZ_MODE, &(!!enable_charger_hiz_mode));
                        battery_log (BAT_LOG_CRTI, "store_Enable_Charger_HIZ_Mode : %d, using hiz mode \n", enable_charger_hiz_mode);
                }
        }
...
static DEVICE_ATTR(Enable_Charger_HIZ_Mode, 0744, show_Enable_Charger_HIZ_Mode, store_Enable_Charger_HIZ_Mode);
#endif
// >>> 2017/05/24-youchihwang. Battery. Release charger HIZ mode switch to user space.
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
    ...
    // <<< 2017/01/17-youchihwang. Battery. Extending battery life. (The phase of 1% -> shutdown)
...
// <<< 2016/11/09-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
static PMU_STATUS mt_battery_CheckRetailDemoMode(void)
{
        PMU_STATUS status = PMU_STATUS_OK;
		
        battery_log (BAT_LOG_CRTI, "FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable : %d \n", FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable);
...
static PMU_STATUS mt_battery_CheckCallState(void)
{
    ...
    // <<< 2017/04/28-youchihwang. Battery. Call mode feature.
    #if 0
...
// <<< 2016/11/21-youchihwang. Battery. FP002219. IEEE 1725 (Battery Safety). Maintenance.
#if defined(CHECK_MAINTENANCE_TIME)
static PMU_STATUS mt_battery_CheckMaintenanceTime(void)
{
	PMU_STATUS    status = PMU_STATUS_OK;

	battery_log(BAT_LOG_CRTI, "[mt_battery_CheckBatteryStatus] maintenance_timer : (%d) secondds\n", BMT_status.maintenance_timer); 

        if(BMT_status.maintenance_timer > 10) {
        
               //
               // Under battery full condition
               //
               // maintenance_timer > 260 hours then a new charging cycle is commenced. 
               //
               //   0 ~ 60   (60) hours. CV = Vmax - 50 mV 
               //  60 ~ 260 (200) hours. CV = Vmax - 100mV 
               // 260 ~ 320  (60) hours. CV = Vmax - 50 mV 
               // 320 ~ 520 (200) hours. CV = Vmax - 100mV 
               // 520 ~ 580  (60) hours. CV = Vmax - 50 mV 
...
static void mt_battery_CheckBatteryStatus(void)
{
    ...
    // <<< 2016/11/09-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
    if (mt_battery_CheckRetailDemoMode() != PMU_STATUS_OK) {
...
static void mt_battery_notify_VBat_check(void)
{
        ...
        // <<< 2017/05/18-youchihwang. (Vbat) over charging protection mmi warning message. 
        #if 0
	...
	battery_log(BAT_LOG_CRTI, "[BATTERY] BATTERY_NOTIFY_CASE_0004_VBAT (%x)\n",
        ...
        if (bat_vol > 4500) {
                g_BatteryNotifyCode |= 0x0008;
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_vol(%d) > 4500mV\n", bat_vol);
        }
        //
        // The phone shall detect when battery voltage is greater than 4.3V,
        // and stop charging the battery. If after 60 seconds or more charging current
        // is not set to 0mA an appropriate message shall be displayed and instructing the
        // user to remove the charger and contact a service center.
        //
        // below 4350 is for noise tolerance. 4350 is 4.35V.
        // below 320 is for noise tolerance. 320 is 32 mA.
        //
        else if (bat_vol > 4350) {
                BMT_status.over_charging_timer += 10;

                if ((BMT_status.over_charging_timer >= 60) && (BMT_status.IBattery > 320)) {
                        g_BatteryNotifyCode |= 0x0008;
                        battery_log(BAT_LOG_CRTI, "[BATTERY] bat_vol(%d) > 4350mV, still charging\n", bat_vol);
                }
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_vol(%d) > 4350mV\n", bat_vol);
        }
        else {
...
// <<< 2017/04/26-youchihwang, Battery. The phone can't show Vcharger warning message.
	...
	battery_charging_control(CHARGING_CMD_GET_VBUS, &vbus);
        battery_log(BAT_LOG_CRTI, "[BATTERY] vbus : %d mV\n", vbus);

	if (is_charger_ovp(vbus)) {
#else
	if (vbus > v_charger_max) {
#endif
		g_BatteryNotifyCode |= 0x0001;
		battery_log(BAT_LOG_CRTI, "[BATTERY] vbus(%d) > %d mV\n",
...
#else
#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
	// <<< 2017/01/23-dangyiwang. Add charger type detection error solution with MTK solution [FAQ15039].
...
void BAT_thread(void)
{
...
#endif
        // <<< 2017/04/26-youchihwang, Battery. The phone can't show Vcharger warning message.
        unsigned int charging_ic_adc_conversion_enable = 1;
        // <<< 2017/04/26-youchihwang, Battery. The phone can't show Vcharger warning message.
	...
	mt_battery_thermal_check();
        ...
        // <<< 2017/04/26-youchihwang, Battery. The phone can't show Vcharger warning message.
	battery_charging_control(CHARGING_CMD_ADC_START, &charging_ic_adc_conversion_enable);
        // >>> 2017/04/26-youchihwang, Battery. The phone can't show Vcharger warning message.
...
int __batt_init_cust_data_from_cust_header(void)
{
	...
	batt_cust_data.usb_charger_current_configured = USB_CHARGER_CURRENT_CONFIGURED;
        ...
        // <<< 2016/12/13-youchihwang, Battery, Increase pc usb port input current limit for SONY ESTA test depends on VARIANT.
        #if 0
                // original code
                batt_cust_data.usb_charger_current = USB_CHARGER_CURRENT;
        #else
                // <<< 2016/12/09-youchihwang, Battery, Increase pc usb port input current limit for SONY ESTA test depends on VARIANT.
                #if defined( USER_BUILD )
                        batt_cust_data.usb_charger_current = USB_CHARGER_CURRENT;
                #else
                        batt_cust_data.usb_charger_current = CHARGE_CURRENT_1000_00_MA;
                #endif
                // >>> 2016/12/09-youchihwang, Battery, Increase pc usb port input current limit for SONY ESTA test depends on VARIANT.
        #endif
        // >>> 2016/12/13-youchihwang, Battery, Increase pc usb port input current limit for SONY ESTA test depends on VARIANT.
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
        ...
        #else
                // modified code
                ret_device_file = device_create_file (&(dev->dev), &dev_attr_Charging_Animation_Battery_Capacity);
        #endif
        // <<< 2017/03/16-youchihwang. Battery. Phone's request : can't power on on 0%.
        /// <<< 2017/05/24-youchihwang. Battery. Release charger HIZ mode switch to user space.
        #if 0
        #else
                ret_device_file = device_create_file (&(dev->dev), &dev_attr_Enable_Charger_HIZ_Mode);
        #endif
        /// >>> 2017/05/24-youchihwang. Battery. Release charger HIZ mode switch to user space.
	}
