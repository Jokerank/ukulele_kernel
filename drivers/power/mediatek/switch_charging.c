...
// <<< 2016/11/24-youchihwang. Battery. FP002219. IEEE 1725 (Battery Safety). Temperature.
	...
	if (g_temp_status == TEMP_ABOVE_POS_60) {
        ...
        battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set phone input current : %d \n; JEITA set battery charging current : %d \n", g_temp_input_CC_value, g_temp_CC_value);
#endif
// >>> 2016/11/24-youchihwang. Battery. FP002219. IEEE 1725 (Battery Safety). Temperature.
}
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	...
	if (charging_full_check() == KAL_FALSE) {
        ...
        // <<< 2016/11/21-youchihwang. Battery. FP002219. IEEE 1725 (Battery Safety). Maintenance.
                ...
                BMT_status.maintenance_timer += BAT_TASK_PERIOD;
        }
        // >>> 2016/11/21-youchihwang. Battery. FP002219. IEEE 1725 (Battery Safety). Maintenance.
...
PMU_STATUS BAT_BatteryHoldAction(void)
{
	...
	#if 0
    if (BMT_status.bat_vol < TALKING_RECHARGE_VOLTAGE || g_call_state == CALL_IDLE || BMT_status.UI_SOC2 < 35) {
	#else
